...
public class Server {
    public static void main(String[] args) throws IOException {
        ...
        panel.addComponent(new Label("Введите IP"));
        final var ipBox = new TextBox().addTo(panel);

        panel.addComponent(new Label("Выберите порт для получения"));
    ...
    public static ThroughputInfo getPackets(String host, int port) throws IOException {
        ...
        try (var socket = new ServerSocket(port, 1, InetAddress.getByName(host))) {
            ...
            try (var reader = new BufferedReader(new InputStreamReader(conn.getInputStream()))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    var throughput = (line.length() / (double)Duration.between(ts, now).getNano());
